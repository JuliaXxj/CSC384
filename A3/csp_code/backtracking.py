...
def FC(unAssignedVars, csp, allSolutions, trace):
    ...
    for val in nxtvar.curDomain():
        ...
        for cnstr in csp.constraintsOf(nxtvar):
            if cnstr.numUnassigned() == 1:
                if FCCheck(cnstr, nxtvar, nxtvar.getValue()) == "DWO":
                    constraintsOK = False
                    break
        if constraintsOK:
            ...
            if new_solns:
                solns.extend(new_solns)
            if len(solns) > 0 and not allSolutions:
                nxtvar.restoreValues(nxtvar, nxtvar.getValue())
                break #don't bother with other values of nxtvar
                      #as we found a soln.
        nxtvar.restoreValues(nxtvar, nxtvar.getValue())
    nxtvar.unAssign()
            ...
            for val in var.curDomain():
                if not c.hasSupport(var, val):
                    ...
                    if var.curDomainSize() == 0:
                        return "DWO"
            if changed:
                # print("changed with {}".format(len(csp.constraintsOf(var))))
...
def GAC(unAssignedVars, csp, allSolutions, trace):
    ...
    nxtvar = unAssignedVars.extract()
    # print(nxtvar, end=" ")
    # print("before GAC:", end=" ")
    # print(nxtvar.curDomain())
    if trace: print("==>Trying {}".format(nxtvar.name()))
    for val in nxtvar.curDomain():
        if trace: print("==> {} = {}".format(nxtvar.name(), val))
        for pv in nxtvar.curDomain():
            if pv != val:
                nxtvar.pruneValue(pv, nxtvar, val)
        nxtvar.setValue(val)
