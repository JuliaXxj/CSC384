...
class QueensTableConstraint(TableConstraint):
    ...
    def __init__(self, name, qi, qj, i, j):
        ...
        solution = self.getAllSatAssignments(name, qi, qj, i, j)
        TableConstraint.__init__(self, name, [qi, qj], solution)

    def getAllSatAssignments(self, name, qi, qj, i, j):
        """
        Return a list of lists of satisfying assignments for variable qi and qj.
        """

        results = []
        qc = QueensConstraint(name, qi, qj, i, j)
        old_qi = qi.curDomain()
        old_qj = qj.curDomain()
        for vali in old_qi:
            qi.setValue(vali)
...
class NValuesConstraint(Constraint):
    ...
    def check(self):
        ...
        for var in self.scope():
            if var.isAssigned() and (var.getValue() in self._required):
                total += 1
        if total < self._lb or total > self._ub:
            return False
        return True
    ...
    def hasSupport(self, var, val):
        ...
        if var not in self.scope():
            return True
        def valsNotOutOfBound(l):
            '''tests a list of assignments which are pairs (var,val)
               to see if they can satisfy the all diff'''
            vals = [val for (var, val) in l]
            total = 0
            for v in self._required:
                total += vals.count(v)
            return self._lb <= total and self._ub >= total
        def partialValsNotOutOfBound(l):
            '''tests a list of assignments which are pairs (var,val)
               to see if they can satisfy the all diff'''
            vals = [val for (var, val) in l]
            total = 0
            for v in self._required:
                total += vals.count(v)
            return self._ub >= total
        varsToAssign = self.scope()
        varsToAssign.remove(var)
        x = findvals(varsToAssign, [(var, val)], valsNotOutOfBound, partialValsNotOutOfBound)
        return x
