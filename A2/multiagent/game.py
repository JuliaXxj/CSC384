...
class Directions:
    ...
    LEFT = OrderedDict([[WEST, SOUTH], [STOP, STOP], [EAST, NORTH], [NORTH, WEST], [SOUTH, EAST]])

    RIGHT = OrderedDict([[NORTH, EAST], [EAST, SOUTH], [STOP, STOP], [SOUTH, WEST], [WEST, NORTH]])

    REVERSE = OrderedDict(
        [[WEST, EAST], [STOP, STOP], [EAST, WEST], [NORTH, SOUTH], [SOUTH, NORTH]]
    )
...
class Game:
    ...
    def __init__(
        self, agents, display, rules, startingIndex=0, muteAgents=False, catchExceptions=False
    ):
        ...
        while not self.gameOver:
            ...
            if "observationFunction" in dir(agent):
                ...
                if self.catchExceptions:
                    try:
                        timed_func = TimeoutFunction(
                            agent.observationFunction, int(self.rules.getMoveTimeout(agentIndex))
                        )
                        try:
                            start_time = time.time()
